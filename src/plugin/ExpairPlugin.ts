...
export default class ExpairPlugin extends Plugin {
  ...
  private addCommands() {
    ...
    const examplesByLang = tuningExamples.reduce(
      (acc, example) => {
...
class ExpandCommand implements Command {
  ...
  constructor(
    private app: App,
    exampleSet: { lang: string; examples: TuningExample[] },
    ...
    this.expander = new GptAbbrevExpander(
      { systemPrompt, expandTextPrompt },
      openai,
      examples,
    );
  }

  public async editorCallback(
    editor: Editor,
  ...
  public async runAnalysis(editor: Editor, originalCallout: boolean) {
    ...
    try {
      const expandedText =
        (await this.expander.analyze(selection)) || "No results from AI!";
...
class YesNoModal extends Modal {
  constructor(
    ...
    private onSubmit: (yesNo: boolean) => Promise<void>,
  ) {
    super(app);
